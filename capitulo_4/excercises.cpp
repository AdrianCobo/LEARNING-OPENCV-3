...
void _1()
{
    cv::Mat image = cv::Mat::zeros(500, 500, CV_8UC(1)); // CV_8U, _8UC1, _8UC(1)
}
...
cv::Mat naive_integrals(const Mat& image)
{
...
cv::Mat fast_integrals(const Mat& image)
{
    cv::Mat integral_image = cv::Mat::zeros(image.rows, image.cols, CV_32FC1);
...
float sub_integral(const cv::Mat& integral_image, cv::Rect r)
{
    auto top = r.y;
...
//    );
    ...
    if (debug_output)
        std::cout << random_image << std::endl << I << std::endl;
        
    auto si = sub_integral(I, { 0, 0, 3, 3 });
    std::cout << "0 0 3 3: " << si << std::endl;
    std::cout << "0 0 1 1: " << sub_integral(I, { 0, 0, 1, 1 }) << std::endl;
    std::cout << "1 1 1 1: " << sub_integral(I, { 1, 1, 1, 1 }) << std::endl;
    std::cout << "1 1 2 2: " << sub_integral(I, { 1, 1, 2, 2 }) << std::endl;
    std::cout << "0 1 3 2: " << sub_integral(I, { 0, 1, 3, 2 }) << std::endl;
}
